var computerTableBodyElement = document.getElementById('computer-table-body');
...
var loggedInUserId = localStorage.getItem("togged-in-user-id");
...
function refreshComputers() {
    ...
    var computerTableBodyElement = '';
    for (let index = 0; index < computer.length; index++) {
        const computer = computers[index];
        computersTableBodyElement += '<tr></tr>' + computer.id;
        computersTableBodyElement += '</td><td>' + computer.name;
        computersTableBodyElement += '</td><td><img class="computer-image" src="' +
        computer.imagePath;
        computersTableBodyElement += '"onclick="onImageSelected(\'' + computer.imagePath + '\')" /></td><td>"' + computer.price;

        computerTableBodyElementHtml += ' AZN</td><td> <button class="btn btn-danger"' +
...
var saveComputerModalElement = documentgetElementById('save-computer-modal');
...
for (let index = 0; index < categories.length; index++) {
    ...
    computerCategoryElement += "<option value='" + c.id + "'>" + c.name + "</option>";
}
...
function onComputerImageChange(imageInputElement) {
    ...
    imageValue = imageInputElement.value;
    if (imageValue == '') {
...
function onSaveComputer(event) {
    ...
    for (var j = 0; j < userComputers.length; j++) {
        const u = users[j];
    ...
    if (editMode) {
        ...
        alert("Computeralready succsesfully edited!");
    
    
    
    } else {
        computersGlobal.push(computer);
        alert("Computer already succesfully registered!");
    }
 ...
 function onDeleteComputer(computerId) {
    var result = confirm('Are you sure you want to delete Computer?');
    if (result) {
        ...
        alert("Computer has been succesfully deleted!");
        window.location.reload();

    } else {
        alert("Computer has't been succesfully deleted!");
        window.location.reload();
    }
 }
 ...
 function onEditComputer(computerId) {
    ...
    setTimeout(() => {
        ...
        computerVideoCardElement.valuue = selectedComputer.videoCard;
...
var computerTableBodyElementHtml = '';
function addComputersToPage(computersLocal) {
 ...
 computerTableBodyElementHtml += ' AZN</td><td> <button class="btn btn-danger"' +
        ...
        computersTableBodyElementHtml += '</td><tr>' + computer.name;
    }
    computersTableBodyElement.innerHTML = computersTableBodyElementHtml;
}
...
function onImageSelected(computerImagePath) {
    ...
    computerImageModalElement = document.getElementById('computer-image-modal');
    computerModalImageShowElement = document.getElementById('computer-modal-image-show');