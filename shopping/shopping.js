var computerCategoriesElement = document.getElementById('computer-categories-div');
 var computersElement= document.getElementById('computers-div');
  ...
  var computerModalImage = document.getElementById('computer-modal-image');
   var computerModalDescription = document.getElementById('computer-modal-descript')
  var  computerModalPrice = document.getElementById('computer-modal-price');
     var computerModalPhone = document.getElementById('computer-modal-phone');
      var computerModaNew = document.getElementById('computer-modal-new');
     var computerModalRam = document.getElementById('computer-modal-ram'); 13
        var computerModalCpu = document.getElementById('computer-modal-cpu'); 14 
        var computerModalDrive = document.getElementById('computer-modal-drive');
         var computerflodalDriveType = document.getElementById('computer-modal-drive-type');
    ...
    var basketModalCloseButtonElement = document.getElementById('basket-modal-table-body');
    ...
    function loadDataFromLocalStorage() {
        var usersString = localStorage.getItem("user");
        var categoriesString = localStorage.getItem("categories");
        var categoriesString = localStorage.getItem("computers");
        if (usersString == null) {
        ...
        if (usersString == null) {
            ...
            computers = JSON.parse(usersString);
...
function loadComputerCategories() {
    ...
    for (var i = 0; i < categories.length; i++) {
        ...
        computerCategoriesElement += "<li class='list-group-item" +
        "list-group-item-action' id='computer-category-" + c.id + "' onclick='onCategorySelected(" + c.id + ")'>" +
        c.name + "</li>";
}
    computerCategoriesElement += "</ul>";
computerCategoriesElement.innerHTML = computerCategoriesElement;
    ...
    if (currentSelectedCategoryId === categoryId) { } else {
        ...
        setTimeout(function () {
            ...
            for (var i = 0; i < computersSelected.length; i++) { 
                ...
                "<div class='computer-image' onclick='onComputerSelected(" + c.id + ")' style='"
                ...
                computersElement.innerHTML = "<h2 class='not-found'>Пo зтой категории нет компьтеpak/h2>";
            }
            ...
            computerModalDescription = selectedComputer.description;
            computerModalPrice = selectedComputer.price + " AZN";
            computerModalPhone = selectedComputer.phone;
            computerModalNew = (selectedComputer.isNew ? 'YES':'NO') ;
            computerModalRAM = selectedComputer.ram + " GB";
            computerModalCPU = selectedComputer.cpu;
            computerModalDrive = selectedComputer.drive + " GB";
            computerModalDriveType = selectedComputer.drive-type == "hdd" ? "HDD" : "SSD";
    ...
    setTimeout(function () {
        ...
        for (let index = 0; index < basketComputers.length; index++) {
            ...
            if (b.computer.id === computerId) {
                b.count++; existInBasket = true; break;
            }
        }
        if(existInBasket) {
           }   else{
...
function onOpenBasket() {
    if (basketComputerCount.length === 0) {
...
function refreshComputersBasket() {
    ...
    basketComputersTableBodyElement = '';
     for (let index = 0; index < basketComputers.length; index++) {
        const b = basketComputers[index];
        ...
        basketComputersTableBodyElementHtml += '</td><td><img class="basket-computer-image" src=' +
            ...
            basketComputersTableBodyElementHtml += '<tr><td>' + b.computer.price;
            ...
            basketComputersTableBodyElementHtml += '</td><td id=">';
...
function computerCountChanged(constInput, computerId) {
...
var customerAdressElement = document.getElementById("customer-adress");
...
function onOrderSubmit(event) {
    ...
    customer.adress = customerAdressElement.value;
    ...
    for (let index = 0; index < userIdList; index++) {
...
function fillCustomerInformation() {
 ...
 var orderCustomer = {};
    if (customerString == null) {
        ...
        customerAdressElement.value = orderCustomer.adress;
...
function allComputersToPage(computersLocal) {
...
function onSearchKeyDown(event) {
    if (event.keyCode == 13) {
         ...
         computersLoading.style.display = 'block';
         setTimeout(function () {
            computersLoading.style.display = 'nonne';
            ...
            console.log(findedComputers.length);
            computers(findedComputers);
            computersElement.style.display = 'block';
            if (findedComputers.length == 0) { computersElement.innerHTML = "<h2 class='not found>No results were found for your request!</h2>";}
            allComputersLoaded = false;
            ...
            if (distanceToBottom < clientHeight + 150) {
                ...
                setTimeout(function () {
                    ...
                    if (computersSelectedGlobal.length <= (begin + length)) {
                        ...
                        allComputersToPage(nextComputers);
